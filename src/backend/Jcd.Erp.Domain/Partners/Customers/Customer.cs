...
public class Customer : BaseAuditableEntity
{
        ...
        string? notes = null)
    {
        if (tenantId == Guid.Empty)
            return Result.Failure<Customer>("Customer.TenantRequired");

        if (string.IsNullOrWhiteSpace(code))
            return Result.Failure<Customer>("Customer.CodeRequired");

        if (string.IsNullOrWhiteSpace(legalName))
            return Result.Failure<Customer>("Customer.LegalNameRequired");
    ...
    {
        if (string.IsNullOrWhiteSpace(code))
            return Result.Failure("Customer.CodeRequired");

        if (string.IsNullOrWhiteSpace(legalName))
            return Result.Failure("Customer.LegalNameRequired");
